            ...
            <div class="quick-start">
              ...
              <ul class="quick-start-ul">
                <template v-for="menu in quickMenuList">
                  <li :key="menu.entId">
                    <router-link :to="menu.menuUri" tag="span">{{
                      menu.entName
                    }}</router-link>
                  </li>
                </template>
...
export default {
  ...
  methods: {
    ...
    // 交易统计，日期选择器，关闭按钮点击事件
    iconClick(dates) {
      this.searchData.createdStart = moment()
        .subtract(7, "days")
    ...
    disabledDate(current) {
      // 当天之前的三十天，可选。 当天也可选
      return (
        current < moment().subtract(32, "days") ||
    ...
    skeletonClose(that) {
      ...
      that.skeletonReqNum++;
      that.skeletonReqNum >= 4
        ? (that.skeletonIsShow = false)
        : (that.skeletonIsShow = true);
    },
  },
  computed: {
    // 快速菜单集合
  ...
  mounted() {
    ...
    this.mainTips.helloTitle =
      `${timeFix()}，` + this.$store.state.user.userName;
    this.init();
    // 去掉交易统计 日期选择框，原生的边框
